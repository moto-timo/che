...
public class TargetsPresenter implements TargetsView.ActionDelegate {
    ...
    private Target createTarget(String targetName, String targetCategory) {
        final CategoryPage page = categoryPageRegistry.getCategoryPageByCategory(targetCategory);

        final Target target;
        if (page != null) {
            page.setDelegate(this);
            target = page.getTargetFactory().createTarget(targetName);
            target.onRestore();
        } else {//set default
            target = new TargetDefaultImpl();
    ...
    public void onTargetSelected(final Target target) {
        ...
        final CategoryPage page = categoryPageRegistry.getCategoryPageByCategory(target.getCategory());
        if (page == null) {
            view.showHintPanel();
            return;
        }
    ...
    public void onAddTarget(String type) {
        final CategoryPage page = categoryPageRegistry.getCategoryPageByCategory(type);
        if (page != null) {
            Target defaultTarget = page.getTargetFactory().createDefaultTarget();
            addTarget(defaultTarget);
            defaultTarget.onSelect();
        }
    }
    ...
    public void onDeleteTarget(Target target) {
        ...
        final CategoryPage page = categoryPageRegistry.getCategoryPageByCategory(type);
        if (page != null) {
            page.getTargetFactory().deleteTarget(target);
        }
    }
